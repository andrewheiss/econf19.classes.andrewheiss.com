...
[Download the slides from today's class](/slides/PMAP-8141_2019-10-24.pdf).

<figure>
[![First slide](/images/slides/slides_2019-10-24.png)](/slides/PMAP-8141_2019-10-24.pdf)
</figure>
...
results_summary <- results %>%
  mutate(pct_world = total / sum(total)) %>%
...
results_summary %>%
  mutate(total = scales::dollar(total),
         pct_world = scales::percent(pct_world)) %>% 
  select(Country = team, `Total earnings` = total, `% of world earnings` = pct_world) %>% 
...
This distribution of wealth created a fairly equal society, with a Gini coefficient of `r results_summary %>%  pull(total) %>% ineq::Gini() %>% round(2)`.
...
results_gini <- get_lorenz(results_summary$total)
